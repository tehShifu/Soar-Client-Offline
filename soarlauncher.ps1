...
if (-Not (Test-Path "$soarDir\libraries\SoarClientOld.jar")) {
  Write-Output "Downloading SoarOld jar
  "
  Import-Module BitsTransfer
  Start-BitsTransfer "https://github.com/tehShifu/Soar-Client-Offline/releases/download/Libraries/SoarClientOld.jar" "$soarDir\libraries\SoarClientOld.jar"
}else {
  Write-Output "Soar Jar already exists"
  }
...
if ($gdirQ -eq 'y') { 
  ...
  if (Test-Path -Path "$gameDir\soar") {
    Write-Output "Soar folder exists already, proceeding.
    "
  }else {
    Write-Output "Please wait while we set up your custom directory...
    "
    ...
    Write-Output "Game directory set to $gameDir
    "
    }
}else {
  $gameDir = "$soarDir\game"
  Write-Output "Using default soar game directory
  "
}
...
if ($skinQ -eq 'y') { 
  if (-Not (Test-Path "$soarDir\libraries\authlib-1.5.21.jar")) {
    Write-Output "Downloading lib for Skin support
    "
    ...
    Write-Output "Downloaded libs
    "
    $playerName = Read-Host -Prompt "Your username [must match Ely.by account] "
    $skinStatus = "Enabled"
  }else {
    Write-Output "Skin libs already downloaded.
    "
    $skinStatus = "Enabled"
    $playerName = Read-Host -Prompt "Your username [must match Ely.by account] "
    }
}else {
  Write-Output "Proceeding without skin support"
